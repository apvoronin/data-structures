...
- **Порядок обработки**: FIFO принцип может быть ограничением для задач с приоритетом.
- **Производительность**: В увеличении размера массива может потребоваться O(n).
- **Управление памятью**: Утечки памяти при использовании связного списка, если не освобождать память.
